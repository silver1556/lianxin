...
clean-images:
	docker rmi $$(docker images -q lianxin-* 2>/dev/null) 2>/dev/null || true
	docker image prune -f

# Database operations
backup:
	@echo "Creating database backup..."
	docker exec lianxin-mysql-prod mysqldump -u root -p${MYSQL_ROOT_PASSWORD} ${DB_NAME} > ./backups/mysql/backup_$$(date +%Y%m%d_%H%M%S).sql
	@echo "Backup completed"

restore:
	@echo "Restoring database from backup..."
	@read -p "Enter backup file name: \" backup_file; \
	docker exec -i lianxin-mysql-prod mysql -u root -p${MYSQL_ROOT_PASSWORD} ${DB_NAME} < ./backups/mysql/$$backup_file
