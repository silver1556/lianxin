...
const config = {
  development: {
    ...
    enableOfflineQueue:
      parseInt(process.env.REDIS_ENABLE_OFFLINE_QUEUE) || false, // Important: disable offline queue to fail fast
    ...
    // Security
    enableTLS: process.env.REDIS_TLS_ENABLED || false,
    tls:
  ...
  test: {
    host: process.env.REDIS_HOST_TEST || "redis",
  ...
  production: {
    ...
    lazyConnect: parseInt(process.env.REDIS_LAZY_CONNECT) || false,
    keepAlive: parseInt(process.env.REDIS_KEEP_ALIVE) || 30000,
...
const clusterConfig = {
  enabled: process.env.REDIS_CLUSTER_ENABLED || false,
  ...
  // Cluster options
  enableReadyCheck: process.env.REDIS_CLUSTER_READY_CHECK || true,
  redisOptions: {
    password: process.env.REDIS_PASSWORD,
    connectTimeout: parseInt(process.env.REDIS_CONNECT_TIMEOUT) || 10000,
    commandTimeout: parseInt(process.env.REDIS_COMMAND_TIMEOUT) || 5000,
