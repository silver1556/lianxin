...
class SettingsService {
   ...
   */
  async requestPhoneChangeOtp(userId, newPhone, countryCode, ipAddress) {
    try {
      ...
      if (existingUser) {
        throw AuthError.duplicatePhone("Phone number already registered");
      }

      // Send OTP
      const result = await otpService.sendPhoneChangeOtp(
        formattedPhoneE164,
        countryCode,
        userId,
        ipAddress
  ...
  async changePhoneNumber(userId, phoneChangeData) {
    ...
    try {
      ...
      // Check if OTP was issued for this phone number
      if (otpVerification.phone !== formattedPhoneE164) {
        throw AuthError.invalidOTP(
          "Phone number does not match the one used for OTP verification"
        );
      }
