...
class RateLimitMiddleware {
  ...
  async _doInitializeRedisStore() {
    try {
      ...
      while (!redisClient.isReady() && Date.now() - startTime < maxWaitTime) {
        await new Promise((resolve) => setTimeout(resolve, 100));
      }

      if (!redisClient.isReady()) {
        throw new Error(
          "Redis client not ready for rate limiting store initialization"
        );
      }
  ...
  createRateLimit(options) {
    const defaultOptions = {
      store: this.getStore(), // Use safe store getter
