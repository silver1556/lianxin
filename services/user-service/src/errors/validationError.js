...
class ValidationError extends AppError {
    ...
    return {
      ...
      message:
        this.validationErrors.length > 0
          ? this.validationErrors[0].message
          : this.message, // first error message
      genericMessage: this.genericMessage || "Validation failed", // global form message
    };
  }
  ...
  static outOfRange(
    field,
    message = `${field} is out of valid range`,
    value = null,
    constraint = null
  ) {
    return new ValidationError(message, null, field).addFieldError(
  ...
  static invalidPhoneNumber(phone, message = "Invalid phone number format") {
    return new ValidationError(message, null, "phone").addFieldError(
      "phone",
      message,
      phone,
      "phone_format"
  ...
  static invalidPassword(
    ...
    return new ValidationError(message, null, "password").addFieldError(
      "password",
      message,
      null,
      "password_strength"
  ...
  /**
   * Static method to create password reused error
   */
  static passwordReuse(message = "Used password cannot be reused") {
    return new ValidationError(message, 400, "PASSWORD_REUSED");
  }
  ...
  static invalidOTP(message = "Invalid or expired OTP") {
    return new ValidationError(message, null, "otp_code").addFieldError(
      "otp_code",
      message,
      null,
      "otp_validation"
  ...
  static invalidAge(
    ...
    return new ValidationError(message, null, "birth_date").addFieldError(
      "birth_date",
      message,
      age,
      "age_range"
