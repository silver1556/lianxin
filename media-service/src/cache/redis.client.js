...
class RedisClient {
  ...
  _updateMetrics(type, data = {}) {
    switch (type) {
      ...
      case "error":
        this.metrics.errors++;
        const errorType = data.error.code || data.error.name || "UNKNOWN_ERROR";
  ...
  async _performHealthCheck() {
    ...
    try {
      ...
      if (this.config.health.commands.info) {
      ...
      if (this.config.health.commands.memory) {
      ...
      // Attempt reconnection if health check fails consistently
      const healthCheckErrors =
        this.metrics.errorsByType.get("HEALTH_CHECK") || 0;
      if (healthCheckErrors > this.config.health.retryCount) {
        logger.warn(
          "Multiple Redis health check failures, attempting reconnection"
        );
  ...
  async cachePartialHash(
    ...
    if (!ttlValue && this.config.cache.ttl.hasOwnProperty(subservice)) {
